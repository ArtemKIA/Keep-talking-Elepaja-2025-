...
/* Port A pins: status LEDs and a spare input */
#define BUTTON_PIN_A  0x08          // GPA3 (input, active-low)
#define BUTTON_LED_MASK_A 0x07      // GPA0..2 used for Game mistake LEDs
 ...
 *
 * B0–B3: outputs to LEDs
 * B4–B7: inputs, pull-ups on, active-low buttons.
 */
#define BUTTON_LED_MASK   0x0F      // B0..B3 = game LEDs

/* 74HC595 + 4-digit 7-segment driver */
#define LATCH_PIN  GPIO_NUM_9
#define DATA_PIN   GPIO_NUM_18
#define CLOCK_PIN  GPIO_NUM_10

/* On-board indicator LED and buzzer */
#define LED_PIN    GPIO_NUM_45
#define BUZZER_PIN GPIO_NUM_4

/* 7-segment bitmaps: digit select and segments */
static const uint8_t s_digit_map[4] = {
    0b00001110,   // digit 0 enable
    0b00001101,   // digit 1 enable
    0b00001011,   // digit 2 enable
    0b00000111    // digit 3 enable
};
...
/* game timer (shared with 7-segment output) */
...
/* button-sequence (“Simon”) game parameters */
#define BUTTON_GAME_MAX_STEPS   16
#define BUTTON_GAME_WIN_ROUNDS  6

/* one-shot flag for reporting a mistake back to main loop */
static bool button_mistake = false;

/* compact state for the button-sequence mini-game */
typedef struct {
    uint8_t  seq[BUTTON_GAME_MAX_STEPS];  // random sequence [0..3]
    int      round;                       // current visible length (1..N)
    int      input_index;                // how many steps entered this round
    uint8_t  last_buttons;               // edge detection
    bool     started;                    // sequence has been generated
    bool     pattern_showing;           // true while LEDs play pattern
} button_game_t;
...
/* configure MCP23017 directions, pull-ups and initial outputs */
...
/* ========== 7-segment driver (74HC595) ========== */

/* shift out a byte MSB-first (bitOrder = 1) or LSB-first (0) */
...
/* drive a single 7-segment digit (multiplexed) */
static void showDigit(int pos, int num, bool withDot)
{
    ...
    /* first shift which digit to enable, then the segment pattern */
...
/* render s_timerSeconds as MM:SS on 4-digit 7-segment */
static void displayTime7seg(void)
{
    ...
    showDigit(1, d2, true);   // colon = decimal point
...
/* short beeps with LED for feedback */
...
/* called every time a full minute elapses */
...
/* ========== Button-sequence mini-game ========== */

/* update LED pattern on Port B (B0..3) */
static void button_game_set_leds(uint8_t mask)
{
    mcp_write_reg(GPIOB, (uint8_t)(mask & BUTTON_LED_MASK));
}

/* flash a single step in the pattern (0..3 = which LED) */
...
/* replay the current pattern to the player */
...
/* reset internal state, but do not start the game yet */
...
/* main state machine for the button mini-game.
   Called frequently from game_update(). */
static void button_game_update(void)
{
    ...
    /* read button bank (Port B) */
    uint8_t bval = 0;
    if (mcp_read_reg(GPIOB, &bval) != ESP_OK) {
        return;
    }

    /* buttons are on B4..B7, active-low */
    uint8_t raw_buttons = (uint8_t)((~bval >> 4) & 0x0F);

    /* LED feedback: mirror buttons while not replaying pattern */
    ...
    if (s_button_game.pattern_showing) {
        return;   // ignore input while pattern is playing
    }

    /* game not started yet: first press seeds and starts the sequence */
    if (!s_button_game.started) {
        ...
        for (int i = 0; i < BUTTON_GAME_MAX_STEPS; ++i) {
            s_button_game.seq[i] = (uint8_t)(esp_random() % 4);
        }
    ...
    /* map button bitmask to index 0..3 */
    int pressed = -1;
    if (new_presses & 0x1)      pressed = 0;
    else if (new_presses & 0x2) pressed = 1;
    else if (new_presses & 0x4) pressed = 2;
    else if (new_presses & 0x8) pressed = 3;
    ...
    if (pressed == expected) {
        ...
        if (s_button_game.input_index >= s_button_game.round) {
            /* round completed, play “success” feedback */
            game_beep_win();
            ...
            if (s_button_game.round > BUTTON_GAME_WIN_ROUNDS) {
                s_game_won = true;
                button_game_set_leds(0);
                ESP_LOGI(TAG, "Button game completed (rounds=%d)",
                         BUTTON_GAME_WIN_ROUNDS);
                return;
            }

            /* show longer pattern for next round */
            button_game_show_pattern(s_button_game.round);
        }
    } else {
        /* mistake: notify main game and replay same pattern */
...
void game_init(i2c_master_bus_handle_t mcp_bus)
{
    ...
    /* attach MCP23017 to given I2C bus */
    ...
    /* GPIO for shift-register + debug LED */
...
/* start a new game with a given total time in seconds */
void game_start(int total_seconds)
{
    s_timerSeconds = (total_seconds > 0) ? (unsigned)total_seconds : 0;
    ...
    /* clear any leftover state on MCP */
    if (s_mcp_dev != NULL) {
...
/* main update: call periodically from app_main loop */
void game_update(void)
{
    if (!s_game_running) return;

    /* timer tick: 1 Hz based on esp_timer */
    ...
    /* 7-segment multiplexing + mini-game logic */
    displayTime7seg();
    button_game_update();
}
...
/* stop game and silence buzzer, clear MCP outputs */
void game_stop(void)
{
    s_game_running = false;
    buzzer_off();
...
/* show number of maze/button mistakes on Port A LEDs (0..3) */
...
/* one-shot flag used by main.c to react to mistakes */
bool button_mistake_done(void)
{
    if (button_mistake) {
        button_mistake = false;  // consume event
        return true;
    }
    return false;
}